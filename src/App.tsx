...
function App() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      const questionListiong = await getQuestionList(totalQuestions, Difficulty.EASY);
      setQuestions(questionListiong);
      setLoading(false);
    };
    fetchQuestions();
  }, []);
    ...
    <main>
      ...
      {userAnswer.length === questionNumber && !gameOver && !loading && !startQuiz ? (
        <div className='greeting-box'>
          <Box boxShadow='base' p='6' rounded='md' bg='white' maxW='560px'>
            ...
            <p>참 또는 거짓으로 대답할 수 있는 {totalQuestions}개의 질문이 제시됩니다.</p>
            <AppButton colorScheme='purple' variant='solid' onClick={startQuizGame} value='Start Quiz Game' />
          </Box>
        </div>
      ) : null}
      ...
      {!loading && !gameOver && startQuiz && (
        <Box boxShadow='base' p='6' rounded='md' bg='white' maxW='560px'>
          ...
          />
          <AppButton
            disabled={userAnswer.length === questionNumber + 1 && questionNumber !== totalQuestions ? false : true}
            colorScheme='purple'
            variant='solid'
      ...
      {gameOver && (
        <>
          <Box boxShadow='base' p='6' rounded='md' bg='white' maxW='560px'>
            ...
            <AppButton colorScheme='purple' variant='solid' onClick={replayQuiz} value='Replay Quiz' width='full' />
          </Box>
        </>
      )}
    </main>
